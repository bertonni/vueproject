<template>
  <div ref="container" class="flex flex-col items-center text-white relative 
    overflow-hidden bg-opacity-80 w-full h-screen transition duration-4500">
    <Header />
    <div class="w-9/12 max-w-2xl min-h-36 flex flex-col items-center pt-4 mt-8 
    rounded" v-if="todoList.length === 0">
    ...
    <div class="w-11/12 sm:w-9/12 max-w-2xl min-h-36 flex flex-col items-center 
      pt-4 mt-2 rounded" v-else>
      <div class="flex items-center gap-5 sm:gap-12 md:gap-24">
        <span class="text-sm sm:text-base md:text-lg">
          Total: {{todoList.length}}
        </span>
        <h1 class="text-2xl flex-grow-2 mb-1 sm:text-3xl md:text-4xl font-bold 
        sm:mb-3">Your Tasks</h1>
        <span class="text-sm sm:text-base md:text-lg">
          Completed: {{ finishedTasks }}
        </span>
      </div>
      <div class="flex flex-col items-center overflow-y-auto max-h-112 w-full
       bg-white bg-opacity-10 rounded py-3 backdrop-filter backdrop-blur-sm">
        <Task v-for="(task, index) in todoList"
          :key=index
          :id="index"
          :task="task"
          @click="redirectToTaskView(index)"
        />
      </div>
    </div>
